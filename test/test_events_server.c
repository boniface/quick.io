...
START_TEST(test_evs_server_format_path_3) {
	...
	check_str_eq(path, "/some/slashes/with/path", "Path concated");
	g_free(path);
...
START_TEST(test_evs_server_format_path_4) {
	...
	gchar *path = evs_server_format_path("/////some///slashes/////", extra);
	check_str_eq(path, "/some/slashes/with/path", "Trailing slash removed");
	g_free(path);
...
START_TEST(test_evs_server_format_path_6) {
	...
	check_str_eq(path, "/these/are/not/valid/with/path", "Bad characters removed");
	g_free(path);
...
START_TEST(test_evs_event_creation_valid_minimal) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_GOOD, "Valid message");

	// Test the event data
	check_str_eq(event.path, "/qio/noop", "Correct event name");
	check_uint16_eq(event.extra->len, 0, "No extra segments");
	check_uint32_eq(event.client_callback, 0, "No callback");
	check(event.data_type == d_plain, "Plain data type set");
...
START_TEST(test_evs_event_creation_valid_single_digit_callback) {
	...
	check_str_eq(event.path, "/qio/noop", "Correct event name");
	check_uint16_eq(event.extra->len, 0, "No extra segments");
	check_uint32_eq(event.client_callback, 1, "No callback");
	check(event.data_type == d_plain, "Plain data type set");
	check_str_eq(event.data, "", "No data set");
...
START_TEST(test_evs_event_creation_valid_callback) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_GOOD, "Valid message");
	check_uint32_eq(event.client_callback, 123, "Callback gotten!");
	check_str_eq(event.data, "", "No data set");
...
START_TEST(test_evs_event_creation_valid_json) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_GOOD, "Valid message");
	check_uint32_eq(event.client_callback, 123, "Callback gotten!");
	check(event.data_type == d_json, "JSON data type set");
	check_str_eq(event.data, "{\"test\":1}", "JSON data recieved");
...
START_TEST(test_evs_event_creation_handle_children) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_GOOD, "Valid message");
	check_ptr_eq(handler, expected, "Got correct handler");
	check_uint32_eq(event.client_callback, 123, "Callback gotten!");
	check(event.data_type == d_plain, "Plain data type set");
	check_str_eq(event.data, "test", "Plain data recieved");

	// Segment testing
	check_uint16_eq(event.extra->len, 1, "1 extra segment");
	check_str_eq(g_ptr_array_index(event.extra, 0), "something", "Correct extra segment");

	_event_free(&event);

	check_ptr_eq(event.path, NULL, "Name free'd");
	check_ptr_eq(event.extra, NULL, "Extra segments free'd");
...
START_TEST(test_evs_event_creation_invalid) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_FATAL, "Invalid message");
...
START_TEST(test_evs_event_creation_invalid_callback_id) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_FATAL, "Invalid message");
	check_uint32_eq(event.client_callback, 0, "Callback parsed");
...
START_TEST(test_evs_event_creation_invalid_handler) {
	...
	check_status_eq(_event_new(message, &handler, &event), CLIENT_ERROR, "Invalid event");
...
START_TEST(test_evs_new_handler_already_exists) {
	...
	check(handler == NULL, "Handler rejected created");
}
END_TEST
...
START_TEST(test_evs_get_handler_6) {
	...
	check_str_eq(g_ptr_array_index(extra, 1), "another", "Correct extra segment");
	check_str_eq(g_ptr_array_index(extra, 2), "segment", "Correct extra segment");
...
START_TEST(test_evs_handler_noop) {
	...
	check_status_eq(evs_server_handle(client), CLIENT_GOOD, "Noop recieved");
	check_size_eq(client->message->buffer->len, 0, "No data set");
...
START_TEST(test_evs_handler_subscribe_already_subscribed) {
	...
	check_size_eq(client->message->buffer->len, 0, "Subscribed!");
...
START_TEST(test_evs_handler_unsubscribe) {
	...
	check_status_eq(evs_server_handle(client), CLIENT_WRITE, "Unsubscribed");
	check_str_eq(client->message->buffer->str, "/qio/callback/456:0:plain=", "Unsubscribed");
...
START_TEST(test_evs_server_server_callback_sane_0) {
	...
	check(evs_server_callback_new(client, _on, NULL, NULL) > 0, "Callback ID sent");
...
START_TEST(test_evs_server_server_callback_evict) {
	...
	check_uint32_eq(calls, MAX_CALLBACKS, "Callbacks called");
	check_uint32_eq(frees, G_N_ELEMENTS(cbs), "Everything free'd");
...
START_TEST(test_evs_server_server_callback_no_callback) {
	...
	check_uint32_eq(callback, 0, "Callback rejected");
	check_uint32_eq(frees, 1, "Everything free'd");
...
START_TEST(test_evs_server_server_callback_client_close) {
	...
	for (guint i = 0; i < G_N_ELEMENTS(cbs); i++) {
		void *t = g_malloc(sizeof(*t));
		cbs[i] = evs_server_callback_new(client, _on, t, _free);
		check(cbs[i] > 0, "Callback accepted");
	}
...
START_TEST(test_evs_server_server_callback_bad_id) {
	...
	check_str_eq(client->message->buffer->str, "/qio/callback/1112:0:plain=qio_error", "Correct response sent");
...
START_TEST(test_evs_server_server_callback_subscribe_to_callback) {
	...
	check_str_eq(client->message->buffer->str, "", "No response sent");
...
START_TEST(test_evs_server_server_callback_subscribe_to_bad_event) {
	...
	check_str_eq(client->message->buffer->str, "/qio/callback/1:0:plain=qio_error", "Error callback sent");
...
START_TEST(test_evs_server_server_callback_fatal) {
	...
	check_status_eq(evs_server_handle(client), CLIENT_FATAL, "Aborted");
}
END_TEST
...
START_TEST(test_evs_server_server_callback_id_overflow) {
	...
	check_status_eq(evs_server_handle(client), CLIENT_ERROR, "Lame callback rejected");
...
START_TEST(test_evs_server_server_callback_chain) {
	...
	status_t _on(client_t *client, void *data, event_t *event) {
		...
		check(callback > 0, "Created server callback");
	...
	check(event.client_callback > 0, "Server callback sent");
	_event_free(&event);
	...
	check_status_eq(evs_server_handle(client), CLIENT_GOOD, "No callback returned");

	check_uint32_eq(calls, 3, "Server callbacks");
	check_uint32_eq(frees, 4, "Free'd callbacks");
...
START_TEST(test_evs_server_server_callback_chain_async) {
	...
	status_t _handler(client_t *client, event_handler_t *handler, event_t *event, GString *response) {
		...
		check(server_callback > 0, "Created server callback");
	...
	g_string_assign(client->message->buffer, "/test/server/cbs:900:plain=");
	check_status_eq(evs_server_handle(client), CLIENT_GOOD, "Callback sent");
	...
	check_str_eq(buff, "\x81""\x1a""/qio/callback/900:1:plain=", "Callback sent");
	...
	memset(&buff, 0, sizeof(buff));
	read(socket, buff, sizeof(buff));
	check_str_eq(buff, "\x81""\x1a""/qio/callback/901:2:plain=", "Callback sent");
	...
	memset(&buff, 0, sizeof(buff));
	read(socket, buff, sizeof(buff));
	check_str_eq(buff, "\x81""\x1a""/qio/callback/902:3:plain=", "Callback sent");
...
START_TEST(test_evs_server_server_callback_uninitialized) {
	...
	check_status_eq(_evs_server_callback(client, NULL, &event, NULL), CLIENT_ERROR, "Callback doesn't exist");
